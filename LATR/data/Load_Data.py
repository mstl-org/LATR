...
class LaneDataset(Dataset):
    ...
    def __init__(self, dataset_base_dir, json_file_path, pkl, args, data_aug=False):
        """
    ...
    def WIP__getitem__(self, idx):
        ...
        if 'openlane' in self.dataset_name:
            ...
            _label_laneline_org, \
            _gt_laneline_category_org, info_dict, \
            _point_cloud_path = self.preprocess_data_from_json_openlane(idx_json_file)
        elif 'once' in self.dataset_name:
            _label_image_path, _label_cam_height, _label_cam_pitch, \
            cam_extrinsics, cam_intrinsics, \
            _label_laneline_org, \
        ...
        # 加载点云数据并确保 float32
        point_cloud = None
        point_cloud = np.fromfile(_point_cloud_path, dtype=np.float32).reshape(-1, 3)
        point_cloud = torch.from_numpy(point_cloud).float()

        matching_data = self.pkl_data['data_list'][idx]
        waymo_lidar2image = matching_data['images']['CAM_FRONT']['lidar2img']
        waymo_intrinsics = matching_data['images']['CAM_FRONT']['cam2img']
        waymo_camera2lidar = np.linalg.inv(matching_data['images']['CAM_FRONT']['lidar2cam'])
        ...
        # 调整 lidar2image 和 camera_intrinsics 以适配裁剪和缩放
        H_crop = np.array(H_crop, dtype=np.float32)
        waymo_lidar2image = np.matmul(H_crop, waymo_lidar2image)  # 适配裁剪和缩放
        waymo_intrinsics = np.matmul(H_crop, waymo_intrinsics)  # 适配裁剪和缩放
        if self.data_aug:
            aug_mat = np.array(aug_mat, dtype=np.float32)
            waymo_lidar2image = np.matmul(aug_mat, waymo_lidar2image)  # 适配数据增强
            waymo_intrinsics = np.matmul(aug_mat, waymo_intrinsics)  # 适配数据增强

        # 转换为 PyTorch 张量
        waymo_lidar2image = torch.from_numpy(waymo_lidar2image.astype(np.float32))
        waymo_intrinsics = torch.from_numpy(waymo_intrinsics.astype(np.float32))
        waymo_camera2lidar = torch.from_numpy(waymo_camera2lidar.astype(np.float32))
        ...
        for i, lane in enumerate(gt_lanes):
            ...
            xs, zs, vis = resample_laneline_in_y(lane, self.anchor_y_steps, out_vis=True)
            ...
            xs_dense, zs_dense, vis_dense = resample_laneline_in_y(
                lane, self.anchor_y_steps_dense, out_vis=True)
        ...
        extra_dict['seg_label'] = seg_label  #二分类任务：车道线 vs. 非车道线
        extra_dict['seg_idx_label'] = seg_idx_label  #实例级别的分割信息
        ...
        extra_dict['point_cloud'] = point_cloud  # 添加点云数据
        extra_dict['waymo_lidar2image'] = waymo_lidar2image  # 适配后的矩阵
        extra_dict['waymo_intrinsics'] = waymo_intrinsics  # 适配后的矩阵
        extra_dict['waymo_camera2lidar'] = waymo_camera2lidar  # 不需调整
        if self.data_aug:
