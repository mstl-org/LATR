...
class LATRHead(nn.Module):
    def __init__(self, args,
        ...
        self.trans_params = dict(
            ...
            z_region=[position_range[2], position_range[5]]) #[-10, 103], [10, 103], [-10, 3], [10, 3], [-10,10]
        ...
        self.pc_range = position_range #[-50, 3, -10, 50, 103, 10.]
    ...
    def forward(self, input_dict, is_training=True):
        output_dict = {}
        img_feats = input_dict['x'] #[2,256,90,120] 前视图特征

        if not isinstance(img_feats, (list, tuple)):
            img_feats = [img_feats]

        sparse_output = self.sparse_ins(
            img_feats[0], #[2,256,90,120]
            lane_idx_map=input_dict['lane_idx'], #[2,20,360,480]
            input_shape=input_dict['seg'].shape[-2:], #[360,480]
            is_training=is_training)
        # generate 2d pos emb
        B, C, H, W = img_feats[0].shape #[2,256,90,120]
        masks = img_feats[0].new_zeros((B, H, W))

        # TODO use actual mask if using padding or other aug
        sin_embed = self.positional_encoding(masks) #[2,256,90,120],正弦/余弦位置编码
        sin_embed = self.adapt_pos3d(sin_embed)

        # init query and reference pt
        query = sparse_output['inst_features'] #[2,40,256]
        # B, N, C -> B, N, num_anchor_per_line, C
        query = query.unsqueeze(2) + self.point_embedding.weight[None, None, ...] #[2,40,256]+[20,256] = [20,40,20,256]
       
        query_embeds = self.query_embedding(query).flatten(1, 2) #[2,800,256]
        query = torch.zeros_like(query_embeds)
        reference_points = self.reference_points(query_embeds) #[2,800,2]
        reference_points = reference_points.sigmoid()
        mlvl_feats = img_feats #[2,256,90,120]
        ...
        for lvl, feat in enumerate(mlvl_feats):
            bs, c, h, w = feat.shape
            spatial_shape = (h, w)
            feat = feat.flatten(2).permute(2, 0, 1) #[10800, 2, 256]
            feat = feat + self.level_embeds[None, lvl:lvl+1, :].to(feat.device) #[10800,2,256]+[1,256]=[10800,2,256]
            spatial_shapes.append(spatial_shape)
            feat_flatten.append(feat)
            mlvl_masks.append(torch.zeros((bs, *spatial_shape),
                                           dtype=torch.bool,
                                           device=feat.device)) #[2,90,120]
        ...
        outs_dec, project_results, outputs_classes, outputs_coords = \
            self.transformer(
                feat_flatten, None, #[10800,2,256]展平的前视图特征图,None
                query, query_embeds, pos_embed,#[2,800,256]全0查询张量,[2,800,256]查询嵌入,None
                reference_points=reference_points,#[2,800,2]
                reg_branches=self.reg_branches,
                cls_branches=self.cls_branches,
                img_feats=img_feats,#[2,256,90,120]原始特征图
                lidar2img=input_dict['lidar2img'],#[2,4,4]
                pad_shape=input_dict['pad_shape'],#(720,960)
                sin_embed=sin_embed, #[2,256,90,120]正弦位置编码
                spatial_shapes=spatial_shapes, #(90,120)
                level_start_index=level_start_index, #(0)
                mlvl_masks=mlvl_masks, #(2,90,120)masks
                mlvl_positional_encodings=mlvl_positional_encodings, #None
                pos_embed2d=pos_embed2d, #None
                image=input_dict['image'], #[2,3,720,960]
                **self.trans_params)
        ...
        all_line_preds[..., 1] = (all_line_preds[..., 1]
            * (self.pc_range[5] - self.pc_range[2]) + self.pc_range[2]) #反归一化 x 和 z 坐标：
        ...
        all_line_preds = all_line_preds.permute(0, 1, 2, 5, 3, 4) #[2,2,40,3,20,1]
        all_line_preds = all_line_preds.flatten(3, 5) #[2,2,40,60]

        output_dict.update({
            'all_cls_scores': all_cls_scores, #[2,2,40,21]
            'all_line_preds': all_line_preds, #[2,2,40,60]
        })
    ...
    def get_loss(self, output_dict, input_dict):
        ...
        def single_layer_loss(layer_idx):
            gcls_pred = all_cls_pred[layer_idx] #[2,40,21]
            glane_pred = all_lane_pred[layer_idx] #[2,40,60]

            glane_pred = glane_pred.view(
                glane_pred.shape[0],
                self.num_group,
                self.num_query,
                glane_pred.shape[-1]) #[2,1,40,60]
            gcls_pred = gcls_pred.view(
                gcls_pred.shape[0],
                self.num_group,
                self.num_query,
                gcls_pred.shape[-1]) #[2,1,40,21]
            ...
            for b_idx in range(len(matched_indices[0])):
                for group_idx in range(self.num_group):
                    ...
                    pos_lane_pred = lane_pred[b_idx][pred_idx] #[8,60]
                    gt_lane = gt_lanes[b_idx][gt_idx] #[8,81]
