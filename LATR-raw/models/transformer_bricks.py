...
def ground2img(coords3d, H, W, lidar2img, ori_shape, mask=None, return_img_pts=False):
    ...
    img_pt = coords3d.flatten(1, 2) @ lidar2img.permute(0, 2, 1) #[2,10500,4]
    img_pt = torch.cat([
    ...
    # rescale to feature_map size 特征图下的坐标
    ...
    canvas = torch.zeros((B, H, W, 3 + 1),
                         dtype=torch.float32,
                         device=coords3d.device) #[2,90,120,4]
    x = x.long()
    y = y.long()
    ind = (x + y * W) * valid.long() #从二维 (H, W) 特征图 flatten 成一维 [H * W] 后的索引[2,10500]
    # ind = torch.clamp(ind, 0, H * W - 1)
    ind = ind.long().unsqueeze(-1).repeat(1, 1, canvas.shape[-1]) #[2,10500,4]
    canvas = canvas.flatten(1, 2) #[2,10800,4]
    target = coords3d.flatten(1, 2).clone() #[2,10500,4]
    scatter_mean(target, ind, out=canvas, dim=1)
    canvas = canvas.view(B, H, W, canvas.shape[-1]
        ).permute(0, 3, 1, 2).contiguous()
    canvas[:, :, 0, 0] = 0 #[2,4,90,120]
    return canvas
...
class MSDeformableAttention3D(BaseModule):
    ...
    def forward(self,
        ...
        value = value.view(bs, num_value, self.num_heads, -1) #[2,10800,4,64]

        sampling_offsets = self.sampling_offsets(query).view(
            bs, num_query, self.num_points_per_anchor,
            self.num_heads, self.num_levels, self.num_points, 2) #[2,800,1,4,1,8,2]
        attention_weights = self.attention_weights(query).view(
            bs, num_query, self.num_points_per_anchor,
            self.num_heads, self.num_levels * self.num_points) #[2,800,1,4,8]
        ...
        attention_weights = attention_weights.view(bs, num_query,
                                                   ...
                                                   self.num_heads,
                                                   self.num_levels,
                                                   self.num_points) #[2,800,1,4,1,8]

        reference_points = reference_points.view(
            bs, self.num_query, self.num_anchor_per_query, -1, 2) #[2,40,20,1,2]
        ref_pt3d = torch.cat([
        ...
        sampling_locations = self.ref_to_lidar(ref_pt3d, pc_range, not_y=True) #[2,40,20,1,3]
        ...
        sampling_locations2d = sampling_locations2d.view(
            *sampling_locations.shape[:-1], 2) #[2,40,20,1,2]
        ...
        sampling_locations2d = sampling_locations2d.permute(0, 1, 2, 4, 5, 6, 3, 7) #[2, 40, 20, 4, 1, 8, 1, 2]
        attention_weights = attention_weights.permute(0, 1, 3, 4, 5, 2) #[2, 800, 4, 1, 8, 1]
        sampling_locations2d = sampling_locations2d.flatten(-3, -2) #[2, 40, 20, 4, 1, 8, 2]
        attention_weights = attention_weights.flatten(-2) / self.num_points_per_anchor #[2, 800, 4, 1, 8]

        xy = 2
        num_all_points = sampling_locations2d.shape[-2] #8

        sampling_locations2d = sampling_locations2d.view(
            bs, num_query, self.num_heads, self.num_levels, num_all_points, xy) #[2, 800, 4, 1, 8, 2]

        if torch.cuda.is_available() and value.is_cuda:
            output = MultiScaleDeformableAttnFunction.apply(
                value, spatial_shapes, level_start_index, sampling_locations2d,
                attention_weights, self.im2col_step) #[1, 10800, 4, 64] 64
        else:
...
class LATRTransformerDecoder(TransformerLayerSequence):
    ...
    def forward(self, query, key, value,
        ...
        # 这段代码是 LATR 中关键的 BEV → 图像对齐步骤，它通过投影和位置编码将 BEV 空间上的 3D 网格，变成图像空间中的位置嵌入，供 Transformer cross-attention 使用，实现图像特征与空间结构对齐。
        batch_size = query.shape[1]
        xmin = top_view_region[0][0] - self.enlarge_length #-10-20
        xmax = top_view_region[1][0] + self.enlarge_length #10+20
        ymin = top_view_region[2][1] - self.enlarge_length #3-20
        ymax = top_view_region[0][1] + self.enlarge_length #103+20
        zmin = z_region[0] #-5
        zmax = z_region[1] #5
        init_ref_3d = generate_ref_pt(
            xmin, ymin, xmax, ymax, init_z,
            bev_w, bev_h, query.device) #(-30,30,70),(-17,127,150)
        init_ref_3d = init_ref_3d[None, ...].repeat(batch_size, 1, 1, 1) #[2,150,70,3]
        ref_3d_homo = F.pad(init_ref_3d, (0, 1), value=1) #[2,150,70,4]齐次坐标
        init_ref_3d_homo = ref_3d_homo.clone()
        init_M = torch.eye(4, device=query.device).float()
        M = init_M[None, ...].repeat(batch_size, 1, 1) #[2,4,4]
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            ground_coords = coords_img_key_pos[:, :3, ...] #ground_coords 是每个 BEV 网格点在图像坐标系下的 [x, y, z] 信息
            ...
            ground_coords[:, 2, ...] = (ground_coords[:, 2, ...] - zmin) / (zmax - zmin) #所有xyz归一化（0，1）
            ground_coords = inverse_sigmoid(ground_coords) #[2,3,90,120] 反sigmoid
            key_pos = self.position_encoder(ground_coords) #[2,256,90,120]
            ...
            if self.post_norm is not None:
                intermediate.append(self.post_norm(query)) #[800,2,256]如果设置了 post_norm，则将每层 Decoder 输出的 query 做一次归一化，并保存到 intermediate，供后续使用或监督。
            else:
                intermediate.append(query)

            query = query.permute(1, 0, 2) #[2,800,256]
            tmp = reg_branches[layer_idx](query) #[2,800,3]

            bs = tmp.shape[0]
            # iterative update
            tmp = tmp.view(bs, self.num_query,
                self.num_anchor_per_query, -1, 3) #[2, 40, 20, 1, 3]
            reference_points = reference_points.view(
            ...
            cls_feat = query.view(bs, self.num_query, self.num_anchor_per_query, -1) #[2, 40, 20, 256]
            cls_feat = torch.max(cls_feat, dim=2)[0] #[2,40,256]
            outputs_class = cls_branches[layer_idx](cls_feat) #[2,40,21]
            ...
            query = query.permute(1, 0, 2) #[800,2,256]
...
class LATRTransformer(BaseModule):
    ...
    def forward(self, x, mask, query,
        ...
        query_embed = query_embed.permute(1, 0, 2) #[800,2,256]
        if mask is not None:
        ...
        out_dec, project_results, outputs_classes, outputs_coords = \
            self.decoder(
                query=target, #[800,2,256]全0
                key=memory, #[10800,2,256]
                value=memory, #[10800,2,256]
                key_pos=pos_embed, #None
                query_pos=query_embed, #[800,2,256]
                key_padding_mask=mask.astype(torch.bool) if mask is not None else None, #None
                reg_branches=reg_branches,
                cls_branches=cls_branches,
                reference_points=reference_points, #[2,800,2]
                spatial_shapes=spatial_shapes, #(90,120)
