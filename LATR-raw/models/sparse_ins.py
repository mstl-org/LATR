...
class InstanceBranch(nn.Module):
    def __init__(self, cfg, in_channels, **kwargs):
        ...
        self.inst_convs = _make_stack_3x3_convs(
                            ...
                            out_channels=dim)  #(4,258,256)

        self.iam_conv = nn.Conv2d(
            dim * num_group,
            num_group * num_mask * sparse_num_group,
            3, padding=1, groups=num_group * sparse_num_group)  #(256,160)
    ...
    def forward(self, seg_features, is_training=True):
        ...
        iam = self.iam_conv(seg_features.tile(
            (1, self.num_group, 1, 1)))  #[2,160,90,120]
        if not is_training:
            iam = iam.view(
                iam.shape[0],
                self.num_group,
                self.num_mask * self.sparse_num_group,
                *iam.shape[-2:])  #[2,1,160,90,120]
            iam = iam[:, 0, ...]
            num_group = 1
        else:
        ...
        # BxNxHxW -> BxNx(HW)
        iam_prob = iam_prob.view(B, N, -1) #[2,160,10800]
        normalizer = iam_prob.sum(-1).clamp(min=1e-6)
        iam_prob_norm_hw = iam_prob / normalizer[:, :, None] #[2,160,10800],[0,0,:]描述第 0 个样本的第 0 个通道在 10800 个像素上的分布

        # aggregate features: BxCxHxW -> Bx(HW)xC
        # (B x N x HW) @ (B x HW x C) -> B x N x C
        all_inst_features = torch.bmm(
            iam_prob_norm_hw,
            seg_features.view(B, C, -1).permute(0, 2, 1)) #[2,160,10800]×[2,10800,256] -> [2,160,256]
        ...
        inst_features = all_inst_features.reshape(
            ...
            B, num_group,
            self.num_mask, -1)  # [2,1,40,1024]
        inst_features = F.relu_(
            self.fc(inst_features)) # [2,1,40,256]
        ...
        iam_prob = iam_prob.view(
            ...
            self.sparse_num_group,
            self.num_mask,
            iam_prob.shape[-1]) #[2, 1, 4, 40, 10800]
        iam_prob = iam_prob.mean(dim=2).flatten(1, 2) #[2, 40, 10800]
        inst_features = inst_features.flatten(1, 2) #[2, 40, 256]
        out.update(dict(
            iam_prob=iam_prob, #[2, 40, 10800],这些概率通常表示分割掩码的像素级分布。
            inst_features=inst_features)) #[2, 40, 256],特征是之前通过加权聚合和全连接层降维得到的实例特征。

        if self.training:
            pred_logits = self.cls_score(inst_features) #[2, 40, 21],分类
            pred_kernel = self.mask_kernel(inst_features) #[2, 40, 256],生成掩码核
            pred_scores = self.objectness(inst_features) #[2, 40, 1],预测对象性分数
            out.update(dict(
...
class SparseInsDecoder(nn.Module):
    ...
    def forward(self, features, is_training=True, **kwargs):
        ...
        if is_training:
            mask_features = self.mask_branch(features) #[2, 256, 90, 120]
            pred_kernel = inst_output['pred_kernel'] #[2, 40, 256]
            N = pred_kernel.shape[1]
            B, C, H, W = mask_features.shape

            pred_masks = torch.bmm(pred_kernel, mask_features.view(
            B, C, H * W)).view(B, N, H, W) #[2,40,90,120],表示每个掩码的分割结果
