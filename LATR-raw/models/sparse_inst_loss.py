...
class SparseInstCriterion(nn.Module):
    ...
    def forward(self, outputs, targets, input_shape):
        ...
        indices = self.matcher(outputs_without_aux, targets, input_shape) # “预测与真实掩码” 的最佳一一配对
...
class SparseInstMatcher(nn.Module):
    ...
    def forward(self, outputs, targets, input_shape):
        with torch.no_grad():
            ...
            tgt_masks = F.interpolate(
                tgt_masks[:, None], size=pred_masks.shape[-2:], mode="bilinear", align_corners=False).squeeze(1) #[11,90,120]
